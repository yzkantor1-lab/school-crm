...
// Loosely typed so this runs from both the manual "Run Sync" button (cookie-
// based server client, authenticated staff) and the cron job (service-role
// admin client, no logged-in user) — see app/api/sola/sync/run/route.ts and
// app/api/cron/sola-sync/route.ts.
...
async function selectAll<T>(
  build: (supabase: AnySupabaseClient, from: number, to: number) => PromiseLike<{ data: T[] | null; error: { message: string } | null }>,
...
export type SyncSummary = {
  ...
  donorSummary: Record<string, number>
}
...
// to run repeatedly (including unattended, every 20 minutes via cron — see
// app/api/cron/sola-sync/route.ts): existing sola_sync_customers rows keep
// whatever match state a staff member has already set (student match, donor
// match, "not a student/donor", merged) — a re-sync only refreshes passive
// fields (name, schedule progress) and appends transactions that weren't
// seen before. Importing a staged payment into tuition_payments/donations is
// always a separate, explicit step a human takes on the Sola Sync page.
export async function runSolaSync(supabase: AnySupabaseClient): Promise<SyncSummary> {
  const [customers, schedules, transactions] = await Promise.all([
  ...
  if (newCustomerRows.length) {
    ...
    if (error) throw new Error(`Failed to stage Sola customers: ${error.message}`)
  }
  ...
  if (newPaymentRows.length) {
    ...
    if (error) throw new Error(`Failed to stage Sola payments: ${error.message}`)
  }
  ...
  const summary: Record<string, number> = { matched: 0, needs_review: 0, unmatched: 0, not_a_student: 0, merged: 0 }
  const donorSummary: Record<string, number> = { matched: 0, needs_review: 0, unmatched: 0, not_a_donor: 0 }
  for (const r of statusCounts) {
    if (r.match_status in summary) summary[r.match_status]++
    if (r.donor_match_status in donorSummary) donorSummary[r.donor_match_status]++
  }
