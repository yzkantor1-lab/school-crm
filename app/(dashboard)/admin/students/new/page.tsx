...
export default function NewStudentPage() {
  ...
  const [form, setForm] = useState({
    ...
    came_semester: '', semester_left: '', allergies: '', medical_notes: '', notes: '',
  })
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm border border-slate-100 p-6 space-y-5">
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select value={form.grade_level} onChange={e => set('grade_level', e.target.value)} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
        ...
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1">Semester Came</label>
          <select value={form.came_semester} onChange={e => set('came_semester', e.target.value)}
        ...
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1">Semester Left</label>
          <select value={form.semester_left} onChange={e => set('semester_left', e.target.value)}
            className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
            <option value="">— Select semester —</option>
        ...
        <div className="flex gap-3 pt-2">
          ...
          <Link href="/admin/students" className="text-sm text-slate-600 hover:text-slate-900 px-4 py-2 rounded-lg border border-slate-200 hover:bg-slate-50 transition">Cancel</Link>
        </div>
      </form>
    </div>
  )
